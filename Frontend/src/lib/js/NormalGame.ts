...
export class Ingame implements GamePhase {
  ...
  addCapture(i: number, pano: any) {
    socket.emit("normalGame:" + 'addCapture', i, pano, (response: string | undefined) => {
      console.log(response);
    });
  }

  endGame() {
    socket.emit("normalGame:" + 'endGame', (response: string | undefined) => console.log(response));
  }
}
...
class Score implements GamePhase {
  ...
  goToLobby() {
    socket.emit("normalGame:" + 'lobby', (response: string | undefined) => console.log(response));
  }

  backToGameOver() {
    socket.emit("normalGame:" + 'backToGameOver', (response: string | undefined) => console.log(response));
  }
}
...
class VotingPhase implements GamePhase {
  ...
  saveReportedPanosInDB = async (i: number, reason: string, playerId: string) => {

    try {
      let { data, error } = await supabase.from("reportedPhotospheres").insert({ panoid: this.captures[i].pano.pano.pano, reason, playerid: playerId });
  ...
  goToScore() {
    socket.emit("normalGame:" + 'score', (response: string) => console.log(response));
  }
