...
export class GeoBingoAgainstHumanity {
  ...
  updateOrCreateGamePhase(gameObj: GameObj, typeOfPhase: any) {
    ...
    else { this.currentPhase = new typeOfPhase(gameObj) }
  }
...
export class Score implements GamePhase {
  ...
  backToGameOver() {
    socket.emit('gah:backToGameOver', (response: string | undefined) => console.log(response));
  }
}
...
export class VotingPhase implements GamePhase {
  ...
  saveReportedPanosInDB = async (panoid: string, reason: string, playerId: string) => {

    try {
      // FIXME: captures are stored differently
      let { data, error } = await supabase.from("reportedPhotospheres").insert({ panoid: panoid, reason, playerid: playerId });
