...
import { changeAuth, makeGuestToken, updateTokenAndAuth, verifyAndGetAuth } from "../helpers/tokenstuff";
...
import { checkIfLobby, checkIfHost, checkCallback, checkIfPlayer } from "../helpers/checkers"
...
export default (io: any, socket: MySocket) => {
  ...
  const init = (token: string | undefined, callback: Function) => {
    ...
    callback({ guestToken: guestToken, streamerFrontPage, playerAndGame: socket.player.toObjWithLobby(), geometries: countryNames, bounds: bounds, time: Date.now().toString() })
