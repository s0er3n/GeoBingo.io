...
class Voting {
  ...
  createScore() {
    ...
    for (const key in this.captureStores) {
      ...
      const playerKeyAsJSONString = JSON.stringify(
        this.captureStores[key].player.toObj()
      );
...
export default class GeoBingoAgainstHumanityGame extends BaseGame {
  ...
  makeCapture(pano: Pano, player: Player, pickIndex: number) {
    ...
    if (pickIndex < this.card.pick) {
      ...
      if (!store) {
        this.captures.push(
          new CardAgainstHumanityCaptureStore(pano, player, pickIndex)
        );
      } else {
  ...
  toGameState() {
    ...
    const state = {
      ...
      totalVotes,
      voting,
      overallScore: [...this.overallScore.entries()].map((playerAndScore) => [
        playerAndScore[0].toObj(),
        playerAndScore[1],
      ]),
    };
    console.log(state);
  ...
  start() {
    ...
    this.captures = this.captures.filter(
      (capture) => Object.keys(capture.captures).length !== 0
    );
  ...
  goToLobby() {
    this.gamePhase = "lobby";
    this.voting?.createScoreWithMap().forEach((score, player) => {
