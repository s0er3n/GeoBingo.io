...
const auth = new Auth("d2b03234-7ab3-40eb-9c9f-eb1e28041523", "twitch", "Sören", "test.png")
const emails = [
...
describe('supabase api', function() {
  describe('getbadgesOfUser', function() {
    ...
    it('should work with id without badges', async function() {
      const auth = new Auth("wrong sub", "twitch", "Sören", "test.png")
      await getBadgesOfUser(auth)
  ...
  describe('getSumOfDonations', function() {
    ...
    it("test with mouses id", async () => {
      const auth = new Auth("e590f04d-8fdd-4bff-b7bc-45452c43883d", "twitch", "mouse", "test.png")
      const donationEmails = await getDonationEmails(auth)
      console.log(donationEmails)
      const sum = await getSumOfDonations(donationEmails)
      expect(sum).toBe(30)
    })
    ...
    it('should return 0 on empty email listl', async function() {
      const sum = await getSumOfDonations([])
      expect(sum).toBe(0)
    });
  ...
  describe("updateEquipedSkin ", () => {
    ...
    it('should upsert undefined', async () => {
      ...
      const auth = new Auth("asd-7ab3-40eb-9c9f-eb1e28041523", "twitch", "Sören", "test.png")
      await updateEquipedSkin(auth, 2)
  ...
  describe('get supporter level', function() {
    ...
    it('auth no support level', async function() {
      const auth = new Auth("does not exist", "twitch", "Sören", "test.png")
      await getSupportLevelAsBadgeAndSupportEmojis(auth)
