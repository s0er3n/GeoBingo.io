...
const auth = new Auth(
  "d2b03234-7ab3-40eb-9c9f-eb1e28041523",
  "twitch",
  "Sören",
  "test.png"
);
const emails = ["soeren.michaels@gmail.com"];
...
describe("supabase api", function () {
  ...
  describe("getSumOfDonations", function () {
    ...
    it("test with mouses id", async () => {
      const auth = new Auth(
        "e590f04d-8fdd-4bff-b7bc-45452c43883d",
        "twitch",
        "mouse",
        "test.png"
      );
  ...
  describe("updateEquipedSkin ", () => {
    ...
    it("should upsert undefined", async () => {
      const auth = new Auth(
        "asd-7ab3-40eb-9c9f-eb1e28041523",
        "twitch",
        "Sören",
        "test.png"
      );
  ...
  describe("get supporter level", function () {
    ...
    it("auth no support level", async function () {
      ...
      await getSupportLevelAsBadgeAndSupportEmojis(auth);
      expect(auth.badges.map((b) => b.startsWith("Donator"))).not.toContain(
        true
      );
    });
  });
